...
d3.wordStream = function () {
    ...
    //<editor-fold desc="region exposed methods to test, should be deleted">
    ...
    //<editor-fold desc="Exporting the functions to set configuration data">
    ...
    wordStream.pictureSize = function (_) {
        return arguments.length ? (pictureSize = _, wordStream) : pictureSize;
    };
    wordStream.pictureShape = function (_) {
        return arguments.length ? (pictureShape = _, wordStream) : pictureShape;
    };
    wordStream.pictureFrequency = function (_) {
        return arguments.length ? (pictureFrequency = _, wordStream) : pictureFrequency;
    };
    ...
    function buildBoxes(data) {
        ...
        let totalFrequencies = calculateTotalFrequenciesABox(data);
        let topics = Array.from(new Set(data.map(d => d3.keys(d.words)).flat()));
        ...
        for (let i = 0; i < data.length; i++) {
            boxes.topics.forEach(topic => {
                ...
                while (++di < n) {
                    ...
                    //place the text or the picture
                    if (d.type == "picture") {
                        //Fill red rect/circle to hold the place (we draw at 0, 0 since already translated to the place)
                        if(pictureShape==="circle"){
                            c.beginPath();
                            c.arc(0, 0, pictureSize/2, 0, 2 * Math.PI);
                            c.fill();
                            if (d.padding) c.lineWidth = 2 * d.padding, c.beginPath(), c.arc(0, 0, pictureSize/2, 0, 2 * Math.PI), c.stroke();
                        }else{
                            c.fillRect(-(w >> 1), -(h >> 1), w, h);
                            if (d.padding) c.lineWidth = 2 * d.padding, c.strokeRect(-(w >> 1), -(h >> 1), w, h);
                        }
        ...
        d3.map(data, function (row) {
            ...
            topics.forEach(topic => {
                ...
                row.words[topic].forEach(element => {
                    //If it is a picture, we count it as pictureFrequency
                    totalFrequency += element.type === "picture" ? pictureFrequency : element.frequency;
                });
